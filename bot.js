...
var cdseconds = 5000;
var sendmsg = db.fetch('sendmsg');
if (sendmsg === null) sendmsg = 1;
var advertisersFrom = 0;
var swearers = 0;
var disabled = 0;
var dad = 0;
var cestitao  = db.fetch(`cestitao`);
...
try {
			...
			return message.channel.send(shorten(data.data, 2000 - (message.author.toString().length + 2)));
		} catch (err) {
...
if(command === "ipcheck"){
message.channel.send("Checking IPs from " + message.mentions.users.first().username + " and " + message.author.username);
